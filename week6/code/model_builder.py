...
class Bert(nn.Module):
    def __init__(self, large, temp_dir, finetune=False):
        ...
        self.model = BertModel.from_pretrained(model_name, cache_dir=temp_dir, return_dict=False)
...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):
        ...
        if args.max_pos > 512:
            ...
            self.bert.model.embeddings.register_buffer(
                "position_ids", torch.arange(args.max_pos).expand((1, -1))
            )
    ...
    def forward(self, src, segs, clss, mask_src, mask_cls):
        ...
        sent_clss = top_vec[torch.arange(top_vec.size(0)).unsqueeze(1), clss] 
